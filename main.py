...
def mostrar_resultados(artist_name):
    lista_artistas = buscar_artistas_en_spotify(artist_name)
    if not lista_artistas:
        return f"<h1>No se encontró al artista '{artist_name}'</h1><a href='/'>Volver</a>"
    return render_template('resultados.html', artistas=lista_artistas, busqueda=artist_name)

@app.route('/analizar/<artist_id>/<artist_name>')
def analizar(artist_id, artist_name):
...
def crear_playlist_final():
    ...
    if not track_ids:
        return "Error: No hay canciones para añadir."
    
    user_id = sp_user.current_user()['id']
    nombre_playlist = f"{artist_name} - Canciones Positivas"
    descripcion = f"Canciones positivas de {artist_name} por Positive Finder."
